...
def display_label_with_tooltip(ticker: str | None, name: str | None, cusip: str | None) -> str:
    """生成带 tooltip 的 HTML 展示标签.
    ...
    Returns:
        HTML 字符串，hover 显示完整信息.
    """
    label = display_label(ticker, name)
    tooltip_parts = []
    if name:
        tooltip_parts.append(f"Name: {name}")
    if cusip:
        tooltip_parts.append(f"CUSIP: {cusip}")
...
def is_international_stock(ticker: str | None) -> bool:
    """判断是否为无 ticker 的国际股.
    ...
    Returns:
        True 如果 ticker 为 None 或空字符串.
    """
    return not ticker or not str(ticker).strip()
