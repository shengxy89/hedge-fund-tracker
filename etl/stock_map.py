...
TICKER_TO_SECTOR: dict[str, tuple[str, str]] = {
    ticker: (sector, industry) for _cusip, ticker, _name, sector, industry in STOCK_MAP
}

# (cusip, ticker, name, sector) — mock_data 用（向后兼容旧签名）
MOCK_STOCKS: list[tuple[str, str, str, str]] = [
    (cusip, ticker, name, sector) for cusip, ticker, name, sector, _industry in STOCK_MAP
]
